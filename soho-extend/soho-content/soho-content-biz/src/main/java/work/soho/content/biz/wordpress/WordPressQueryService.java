 ...
 */
@Service
public class WordPressQueryService {
    ...
    public WordPressQueryService(AdminContentService contentService,
    ...
    public List<WordPressPostDto> listPosts(String type, int page, int perPage) {
        PageHelper.startPage(page, perPage);
        List<ContentInfo> contents = contentService.list(new LambdaQueryWrapper<ContentInfo>()
                .orderByDesc(ContentInfo::getId));

        Map<Long, ContentExternalMapping> mappingMap = loadMappings(contents);
        Map<Long, List<Long>> tagMap = loadTagRelations(contents);

        List<WordPressPostDto> result = new ArrayList<>();
    ...
    public List<WordPressTermDto> listCategories(int page, int perPage) {
        PageHelper.startPage(page, perPage);
    ...
    public List<WordPressTermDto> listTags(int page, int perPage) {
        PageHelper.startPage(page, perPage);
    ...
    public List<WordPressMediaDto> listMedia(int page, int perPage) {
        PageHelper.startPage(page, perPage);
    ...
    public List<WordPressUserDto> listUsers(int page, int perPage) {
        PageHelper.startPage(page, perPage);
        List<Long> userIds = collectUserIds();
        List<WordPressUserDto> users = new ArrayList<>();
        for (Long userId : userIds) {
            UserInfoDto user = userApiService.getUserById(userId);
            WordPressUserDto dto = mapper.toUser(user);
            if (dto != null) {
                users.add(dto);
            }
        }
    ...
    public List<WordPressCommentDto> listComments(int page, int perPage) {
        PageHelper.startPage(page, perPage);
