...
3. 处理结果：根据 `tryAcquire()` 的返回结果，决定是继续执行业务逻辑，还是拒绝请求。
...
- **优点**：实现简单，容易理解。
- **缺点**：在时间窗口的边界处可能出现流量突刺，例如窗口切换瞬间流量可接近限制的 2 倍。
- **线程安全**：非线程安全，适用于单线程环境。
...
- `windowSizeInMillis`：时间窗口大小，单位毫秒。
...
- **优点**：限流控制更为平滑，可有效避免边界突刺。
- **此版本实现**：基于 `ZSET` 思想，使用 `TreeMap` 存储请求时间戳和计数。
